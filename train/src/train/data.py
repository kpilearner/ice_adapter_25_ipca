...
class PairedCSVDataset(Dataset):
    """
    Local paired-image dataset (Kontext-like metadata CSV).

    Expected CSV columns by default:
      - `kontext_images`: path to the source/context image (e.g., visible image)
      - `image`: path to the target image (e.g., infrared image)
      - `prompt`: (optional) instruction text

    Each sample is formatted as a diptych `[source | target]` with a right-half mask.
    """
    ...
    def _read_rows(self):
        ...
        if not rows:
            raise ValueError(f"No rows found in metadata CSV: {self.metadata_path}")
        missing = [
            key
            for key in (self.source_key, self.target_key)
            if key not in rows[0].keys()
        ]
        if missing:
            raise ValueError(
                f"Missing required columns {missing} in {self.metadata_path}. "
    ...
    def __getitem__(self, idx):
        row = self.rows[idx]
        src = self._open_rgb(self._resolve_path(row[self.source_key]))
        tgt = self._open_rgb(self._resolve_path(row[self.target_key]))

        src = src.resize((self.condition_size, self.condition_size)).convert("RGB")
        ...
        combined_image.paste(tgt, (self.condition_size, 0))

        mask = Image.new("L", (self.condition_size * 2, self.condition_size), 0)
        draw = ImageDraw.Draw(mask)
        draw.rectangle(
            [self.condition_size, 0, self.condition_size * 2, self.condition_size],
            fill=255,
        )
