...
def _safe_relative_path(path: Path, base: Path | None) -> Path | None:
    if base is None:
        return None
    try:
...
def _build_caption_index(
    ...
    index: dict[str, str] = {}
    for row in rows:
        rel_path = row.get(image_key, "")
        if not rel_path:
            continue
        rel_path = _normalize_csv_path(rel_path)
        if not rel_path:
            continue
        caption = row.get(caption_key, "") if caption_key else ""
        rel_key = _path_key(Path(rel_path))
        index[rel_key] = caption
        abs_path = _resolve_path(base_path, rel_path).resolve()
        index[_path_key(abs_path)] = caption
    return index
...
def _caption_for_path(
    ...
    abs_key = _path_key(image_path.resolve())
    if abs_key in caption_index:
        return caption_index[abs_key]
    for base in (base_path, input_dir):
        rel = _safe_relative_path(image_path, base)
        if rel is not None:
            rel_key = _path_key(rel)
            if rel_key in caption_index:
                return caption_index[rel_key]
    return ""
...
def main() -> int:
    ...
    if args.max_images and args.max_images > 0 and input_dir is None:
        rows = rows[: args.max_images]
    caption_index = _build_caption_index(rows, args.image_key, args.caption_key, base_path)
    ...
    if input_dir is None:
        ...
        for i, row in enumerate(rows):
            ...
            print(f"[{i+1}/{total}] {rel_path} -> {out_path.as_posix()} (seed={seed})")
    else:
        image_paths = _list_images(input_dir, args.recursive)
        if not image_paths:
            raise ValueError(f"No images found under {input_dir.as_posix()}")
        if args.max_images and args.max_images > 0:
            image_paths = image_paths[: args.max_images]
        total = len(image_paths)

        for i, image_path in enumerate(image_paths):
            rel_path = _safe_relative_path(image_path, input_dir) or Path(image_path.name)
            out_path = _output_path(output_dir, input_dir, image_path)
            out_path.parent.mkdir(parents=True, exist_ok=True)
            if args.skip_existing and out_path.exists():
                continue

            caption = _caption_for_path(image_path, caption_index, base_path, input_dir)
            if not caption:
                caption = args.default_caption
            caption = f"{args.caption_prefix}{caption}"
            ...
            seed = _seed_for(rel_path, args.seed, args.seed_mode)
            if args.seed_mode == "incremental":
                seed = int(args.seed) + i
            ...
            print(
                f"[{i+1}/{total}] {rel_path.as_posix()} -> {out_path.as_posix()} (seed={seed})"
            )
