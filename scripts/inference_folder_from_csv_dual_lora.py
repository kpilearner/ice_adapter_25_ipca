...
def _load_lora_weights(
    pipe: FluxFillPipeline, lora_path: str, adapter_name: str | None = None
) -> None:
    ...
    if p.is_file():
        ...
        weight_name = p.name
        try:
            pipe.load_lora_weights(parent, weight_name=weight_name, adapter_name=adapter_name)
        except TypeError:
            if adapter_name is not None:
                try:
                    pipe.load_lora_weights(parent, adapter_name=adapter_name)
                except TypeError:
                    print("[WARN] This diffusers version does not support adapter_name; LoRA may overwrite.")
...
def _set_active_adapters(pipe: FluxFillPipeline, names: list[str], scales: list[float]) -> None:
    if not names:
        return
    if hasattr(pipe, "set_adapters"):
        ...
        return
    if hasattr(pipe, "set_adapter"):
        pipe.set_adapter(names)
...
def main() -> int:
    ...
    parser.add_argument("--lora-path", type=str, default=None, help="(Legacy) single LoRA path.")
    parser.add_argument("--lora-path-1", type=str, default=None, help="Primary LoRA path.")
    parser.add_argument("--lora-path-2", type=str, default=None, help="Secondary LoRA path.")
    parser.add_argument("--lora-name-1", type=str, default="default", help="Adapter name for LoRA-1.")
    parser.add_argument("--lora-name-2", type=str, default="phys", help="Adapter name for LoRA-2.")
    parser.add_argument("--lora-scale-1", type=float, default=1.0, help="Scale for LoRA-1.")
    parser.add_argument("--lora-scale-2", type=float, default=1.0, help="Scale for LoRA-2.")
    ...
    lora_path_1 = args.lora_path_1 or args.lora_path
    lora_path_2 = args.lora_path_2
    active_lora_names: list[str] = []
    active_lora_scales: list[float] = []

    if lora_path_1:
        _load_lora_weights(pipe, lora_path_1, adapter_name=args.lora_name_1)
        active_lora_names.append(args.lora_name_1)
        active_lora_scales.append(float(args.lora_scale_1))
    if lora_path_2:
        _load_lora_weights(pipe, lora_path_2, adapter_name=args.lora_name_2)
        active_lora_names.append(args.lora_name_2)
        active_lora_scales.append(float(args.lora_scale_2))
    _set_active_adapters(pipe, active_lora_names, active_lora_scales)
